...
def process_arguments():
    ...
    mandatoryGroup.add_argument('-U', type=str,nargs='?', metavar='url', dest='url', help='\t\t Single url to test')
    mandatoryGroup.add_argument('-F', type=str, nargs='?', metavar='urlfile', dest='f', help='\t\t Load multiple urls to test from a file')
    mandatoryGroup.add_argument('-R', type=str, nargs='?', metavar='reqfile', dest='reqfile', help='\t\t Load single request to test from a file')

    optionsGroup = parser.add_argument_group('REQUEST OPTIONS')
    optionsGroup.add_argument('-C', type=str, metavar='<cookie>', dest='cookie', help='\t\t HTTP session Cookie header')
    optionsGroup.add_argument('-D', type=str, metavar='<data>', dest='postreq', help='\t\t HTTP request FORM-data')
    #optionsGroup.add_argument('-J', type=str, metavar='<json>', dest='json', help='\t\t Specify request JSON FORM-data')
    optionsGroup.add_argument('-H', type=str, metavar='<header>', action='append', dest='httpheaders', help='\t\t Additional HTTP header(s)')
    optionsGroup.add_argument('-M', type=str, metavar='<method>', dest='method', help='\t\t Request method to use for testing')
    optionsGroup.add_argument('-P', type=str, metavar = '<proxy>', dest='proxyAddr', help='\t\t Use a proxy to connect to the target endpoint')
    optionsGroup.add_argument('--useragent', type=str, metavar= '<agent>', dest='agent', help='\t\t HTTP user-agent header value')
    optionsGroup.add_argument('--referer', type=str, metavar = '<referer>', dest='referer', help='\t\t HTTP referer header value')
    optionsGroup.add_argument('--placeholder', type=str, metavar='<name>', dest='param', help='\t\t Custom testing placeholder name (default is "PWN")')
    optionsGroup.add_argument('--delay', type=int, metavar='<milis>', dest='delay', help='\t\t Delay in miliseconds after each request')
    optionsGroup.add_argument('--max-timeout', type=int, metavar='<seconds>', dest='maxTimeout', help='\t\t Number of seconds after giving up on a response (default 5)')
    optionsGroup.add_argument('--http-ok', type=int, action='append', metavar='<number>', dest='http_valid', help='\t\t Http response code(s) to treat as valid')
    optionsGroup.add_argument('--csrf-param', type=str, metavar = '<param>', dest='csrfParameter', help='\t\t Parameter used to hold anti-CSRF token')
    optionsGroup.add_argument('--csrf-method', type=str, metavar = '<method>', dest='csrfMethod', help='\t\t HTTP method to use during anti-CSRF token page visit')
    optionsGroup.add_argument('--csrf-url', type=str, metavar = '<url>', dest='csrfUrl', help='\t\t URL address to visit for extraction of anti-CSRF token')
    optionsGroup.add_argument('--csrf-data', type=str, metavar = '<data>', dest='csrfData', help='\t\t POST data to send during anti-CSRF token page visit')
    optionsGroup.add_argument('--second-method', type=str, metavar='<method>', dest='secondMethod', help='\t\t Specify method for second order request')
    optionsGroup.add_argument('--second-url', type=str, metavar='<url>', dest='checkUrl', help='\t\t Url for second order request')
    optionsGroup.add_argument('--second-data', type=str, metavar='<data>', dest='secondData', help='\t\t FORM-line data for second-order request')
    ...
    #enumGroup.add_argument('-eS', '--enum-system', action = 'store_true', dest='enum_system', help='\t\t Enumerate operating system')
    #enumGroup.add_argument('-eF', '--enum-files', action = 'store_true', dest='enum_files', help='\t\t Enumerate file system')
    #enumGroup.add_argument('-eU', '--enum-users', action = 'store_true', dest='enum_users', help='\t\t Enumerate existing users')
    #enumGroup.add_argument('-eP', '--enum-proc', action = 'store_true', dest='enum_proc', help='\t\t Enumerate processes and programs')
    #enumGroup.add_argument('-eN', '--enum-net', action = 'store_true', dest='enum_net', help='\t\t Enumerate network configuration')
    #enumGroup.add_argument('-eA', '--enum-all', action= 'store_true', dest='enum_all', help='\t\t Utilize all enumeration options')
    ...
    wordlistGroup.add_argument('-wT', type=str, metavar = '<path>', dest='truncWordlist', help='\t\t Path to wordlist for path traversal modality')
    ...
    #outputOptions.add_argument('-oA', type=str, metavar='<ALLfiles>', dest='allfile', help='\t\t Output to all supported formats')
