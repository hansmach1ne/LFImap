...
def parse_url_from_request_file(file_path, force_ssl=False):
    ...
    try:
        with open(file_path, "r", encoding="latin1") as file:
            ...
            if args.args['force_ssl'] or force_ssl:
...
def get_params_with_param(url):
    ...
    matching_params = [key for key, value in params.items() if args.args['param'] in value]
...
def post_params_with_param(url):
    ...
    params_with_pwn_value = [
        param for param, value in parsed_params.items() if args.args['param'] in value[0]
    ]
...
def getHeadersToTest(dictionary):
    ...
    matching_keys = [
        ...
        if isinstance(value, bytes) and args.args['param'].encode() in value
    ]
