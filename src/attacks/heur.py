...
def test_heuristics(url, post):
    ...
    for test in tests:
        if(args.verbose and xssTest in test): logging.info(colors.blue("[i]") + " Testing for XSS...")
        if(args.verbose and "%0d%0a" in test): logging.info(colors.blue("[i]") + " Testing for CRLF...")
        ...
        if(res and rProtocol+":" in res.text.lower()):
            ...
            if(len(matches) > 0 and vuln == False):
                logging.info("    Positive regex match found in response: " + pattern)
                vuln = True
                if(args.postreq and len(args.postreq) > 1): logging.info(colors.green("[+]") + " XSS -> '" + u + "' -> HTTP POST -> '" + postTest + "' -> reflection in HREF attribute value")
                else: logging.info(colors.green("[+]") + " XSS -> '" + u + "' -> reflection in HREF atribute value")

        if(res and rProtocol + ":" + rNumb1 + rLetter1 + ">" + rLetter2 + "<" + rLetter3 + ";" + rNumb2 + "\"'" + rLetter4 in res.text.lower()):
            vuln = True
            if(args.postreq and len(args.postreq) > 1): logging.info(colors.green("[+]") + " XSS -> '" + u + "' -> HTTP POST -> '" + postTest + "' -> full reflection in response")
            else: logging.info(colors.green("[+]") + " XSS -> '" + u + "' -> full reflection in response")
            ...
            if(res and rProtocol+":" in res.text.lower()):
                ...
                if(len(matches) > 0 and vuln == False):
                    logging.info("    Positive regex match found in response: " + pattern)
                    vuln = True
                    if(args.postreq and len(args.postreq) > 1): logging.info(colors.green("[+]") + " XSS -> '" + u + "' -> HTTP POST -> '" + postTest + "' -> reflection in HREF attribute value. Check if javascript: is allowed")
                    else: logging.info(colors.green("[+]") + " XSS -> '" + u + "' -> reflection in HREF atribute value. Check if javascript: is allowed")
            ...
            if(res and rNumb2 + "\"'" + rLetter4 in res.text.lower()):
                ...
                if(len(matches) > 0 and vuln == False):
                    logging.info("    Positive regex match found in response: " + pattern)
                    vuln = True
                    if(args.postreq and len(args.postreq) > 1): logging.info(colors.green("[+]") + " XSS -> '" + u + "' -> HTTP POST -> '" + postTest + "' -> reflection in tag attribute")
                    else: logging.info(colors.green("[+]") + " XSS -> '" + u + "' -> reflection in tag atribute")
            ...
            if(res and rLetter3 + ";" + rNumb2 in res.text.lower()):
                ...
                if(len(matches) > 0 and vuln == False):
                    logging.info("    Positive regex match found in response: " + pattern)
                    vuln = True
                    if(args.postreq and len(args.postreq) > 1): logging.info(colors.green("[+]") + " XSS -> '" + u + "' -> HTTP POST -> '" + postTest + "' -> reflection in script context")
                    else: logging.info(colors.green("[+]") + " XSS -> '" + u + "' -> reflection in script context")
        
        # CRLF
        if(res and any('13CRLF37' in value for value in res.headers.values()) and any('Lfi' in key for key in res.headers)):
            vuln = True
            if(args.postreq and len(args.postreq) > 1): logging.info(colors.green("[+]") + " CRLF -> '" + u + "' -> HTTP POST -> '" + postTest + "' -> response splitting, 'lfi' header is present")
            else: logging.info(colors.green("[+]") + " CRLF -> '" + u + "' -> response splitting, 'Lfi' header is present")
        ...
        if(vuln): 
            ...
            if("Content-Type" in res.headers):
                logging.info("    Content-Type: " + res.headers["Content-Type"])
            if("Content-Security-Policy" in res.headers):
                logging.info("    Content-Security-Policy: " + res.headers["Content-Security-Policy"])

        if(args.quick or br): break

    if(args.verbose): logging.info(colors.blue("[i]") + " Testing for error-based info leak...")

    if(res and fiErrors[0] in res.text.lower()):
        for i in range(1,len(fiErrors)):
            if(fiErrors[i] in res.text.lower()):
                if(args.postreq and len(args.postreq) > 1): logging.info(colors.green("[+]") + " Info disclosure -> '" + fiErrors[i] + "' error triggered -> '" + u + "' -> HTTP POST -> '" + postTest + "'")
                else: logging.info(colors.green("[+]") + " Info disclosure -> '" + fiErrors[i] + "' error triggered -> '" + u + "'")
                stats["vulns"] += 1

        # Check for Sql errors
        for i in range(len(sqlErrors)):
            if(res and sqlErrors[i] in res.text.lower()):
                if(len(args.postreq) > 1): logging.info(colors.green("[+]") + " Info disclosure -> '" + sqlErrors[i] + "' error detected -> '" + u + "' -> HTTP POST -> '" + postTest + "'")
                else: logging.info(colors.green("[+]") + " Info disclosure -> '" + sqlErrors[i] + "' error detected -> '" + u + "'")
                stats["vulns"] += 1

    # Open redirect check
    if(args.verbose): logging.info(colors.blue("[i]") + " Testing for open redirect...")
    ...
    if(res and loc != None and "/lfi/" in loc):
        ...
        if(loc == "/lfi/a/../" or loc == "http:///lfi/a/../" or loc == "https:///lfi/a/../"):
            if(args.postreq and len(args.postreq) > 1): logging.info(colors.green("[+]") + " Open redirect -> '" + u + "' -> HTTP POST -> '" + postTest.replace("/lfi/a/../", "evil.com") + "'")
            else: logging.info(colors.green("[+]") + " Open redirect -> '" + u.replace("/lfi/a/../", "evil.com") + "'")
            stats["vulns"] += 1
            br = True
        ...
        # Reflection after the relative path
            ...
            if(args.postreq and len(args.postreq) > 1): 
                logging.info(colors.green("[+]") + " Open redirect via relative double slash -> '" + u + "' -> HTTP POST -> '" + postTest.replace("/lfi/a/../", "/evil.com/") + "'")
            else: 
                logging.info(colors.green("[+]") + " Open redirect via relative double slash -> '" + u.replace("/lfi/a/../", "/evil.com/") + "'")
            stats["vulns"] += 1
            br = True

        elif("/a/../" in loc and "/a/../" in urlparse.urlparse(loc).path):
            if(args.postreq and len(args.postreq) > 1):
                logging.info(colors.green("[+]") + " Client-Side path traversal redirect -> '" + u + "' -> HTTP POST -> '" + postTest.replace("/lfi/a/../", "/../arbitrary/endpoint") + "'")
            else: 
                logging.info(colors.green("[+]") + " Client-Side path traversal redirect -> '" + u.replace("/lfi/a/../", "/../arbitrary/endpoint") + "'")
            stats["vulns"] += 1
            br = True
    return