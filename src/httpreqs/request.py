...
def REQUEST(url, headersData, postData, proxy, exploitType, exploitMethod, exploit = False, followRedirect = True, isCsrfRequest = False):
    ...
    try:
        ...
        if(isCsrfRequest):
            if(args.csrfMethod and isCsrfRequest): method = args.csrfMethod
        elif(args.method): method = args.method
        else: method = "GET"
        ...
        if(args.updateCsrfToken):
            if(args.previouscsrf):
                ...
                r = requests.request(method, url, data=postData, headers = headersData, proxies = proxy, verify = False, timeout = timeout, allow_redirects = True)
                input_fields = extract_input_fields(r.text)
                parameters = extract_all_parameters(config.url, config.postreq)
            ...
            if(exploitMethod == "test" or exploitMethod == "RFI"): 
                res = requests.request(method, url, data=postData, headers=headersData, proxies=proxy, verify=False, timeout = timeout, allow_redirects = followRedirect)
            else: res = requests.request(method, url, data=postData, headers=headersData, proxies=proxy, verify=False, allow_redirects = followRedirect)
            ...
            if(args.updateCsrfToken):
                ...
                if(args.csrfParameter and args.csrfParameter in input_fields and input_fields[args.csrfParameter]):
                    args.previouscsrf = input_fields[args.csrfParameter]
                else: args.previouscsrf = False

                if(res): args.previousres = res
            ...
            if(args.checkUrl and not isCsrfRequest):
                if(args.secondMethod): res = requests.request(args.secondMethod, args.checkUrl, data=args.secondData, headers=headersData, proxies=proxy, verify=False, allow_redirects = True)
                else: res = requests.get(args.checkUrl, data=args.secondData, headers=headersData, proxies=proxy, verify=False, allow_redirects = True)
